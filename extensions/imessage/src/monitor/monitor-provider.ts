...
export async function monitorIMessageProvider(opts: MonitorIMessageOpts = {}): Promise<void> {
  ...
  async function handleMessageNowInner(rawMessage: IMessagePayload) {
    ...
    // poll's caption. The poll balloon (rendered with options + a vote cue) is
    // still delivered; only the caption is folded in and dropped, so the agent
    // votes without also answering the caption as a standalone question.
    if (message.poll) {
