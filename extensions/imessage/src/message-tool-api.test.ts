...
describe("iMessage message-tool artifact", () => {
  ...
  it("offers poll-vote when the pollPayloadMessage selector and poll.vote rpc are present", () => {
    // Votes are built with the same pollPayloadMessage balloon initializer as
    // poll-send; the poll.vote rpc proves the binary carries the vote command.
    // There is no separate pollVoteMessage selector in imsg.
    setCachedIMessagePrivateApiStatus("imsg", {
      available: true,
      v2Ready: true,
      selectors: { pollPayloadMessage: true },
