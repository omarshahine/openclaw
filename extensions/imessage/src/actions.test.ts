...
describe("imessage message actions", () => {
  ...
  it("rejects a poll vote when the bridge lacks the poll balloon initializer", async () => {
    // Votes are constructed via the same pollPayloadMessage balloon initializer
    // as poll-send; without it the bridge cannot build the vote.
    const staleStatus = {
      available: true,
      v2Ready: true,
      selectors: {},
