...
See [Automations vs Heartbeat](/automation#scheduled-tasks-cron-vs-heartbeat) for the full decision table. Short version: heartbeat batches periodic checks with full session context on approximate timing (default every 30 minutes); automations are for exact timing, isolated runs, a different model, or one-shot reminders.
...
- [Automations vs heartbeat](/automation#scheduled-tasks-cron-vs-heartbeat)
