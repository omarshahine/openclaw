...
describe("poll vote echo guard", () => {
  ...
  it("suppresses an echo that lands in a later run (new tool instance, same session)", async () => {
    // The live failure: a native poll and its question arrive as separate
    // inbound messages, so the vote and the restatement run in different agent
    // turns with fresh tool instances. A session-scoped record must still catch
    // the follow-up reply.
    ...
    // New tool instance = new run, same conversation. The send/reply guard path
    // is identical; send avoids the reply-dispatch path the test env can't mock.
  ...
  it("does not suppress a later-run echo from a different conversation", async () => {
    ...
    await castBlueVote(voteTool);
    // Different session key = different conversation; must not suppress.
