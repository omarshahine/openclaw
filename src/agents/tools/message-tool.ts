...
// poll and its accompanying question arrive as separate inbound messages and are
// processed in separate agent runs, each with a fresh tool instance. An
// instance-local record would be lost before the follow-up text run, so the
// echo (the agent restating its vote in prose) would leak. Session-scoped +
...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  // Poll-vote echo record lives in the session-scoped map (see
  // recentPollVoteBySession) so it survives the run boundary between the vote
  // and the follow-up text; null session key disables the guard.
